 ...
 *
 *          console.log(respoke['Admin-Token']); "XXXXXX-XXXX-XXXXXXXXX-XXXX"
 *      });
 *
...
function Client(options) {
    ...
    self.auth.appAuthSession = function (opts, callback) {
        ...
        if (!opts.tokenId) {
            return callback(new Error("Cannot authenticate without tokenId"));
        }
    ...
    self.auth.connect = function (opts) {
        ...
        if (!opts['App-Token'] && !opts.endpointId) {
            return self.emit('error', {message: "Need endpointId to connect as an administrator"});
        }
        ...
        self.socket.on('connect', function () {
            ...
            self.wsCall('post', '/endpointconnections', dataBody, function (err, data) {
                if (err) {
                    self.emit('error', err);
                    return;
                }
    ...
    self.wsCall = function (httpMethod, urlPath, data, callback) {
        ...
        if (self.tokens['App-Token']) {
            whichHeader['App-Token'] = self.tokens['App-Token'];
        }
        // Be sure to set own endpointId when not using App-Token.
        else {
            whichHeader = self.tokens;
            // data = data || {};
            // if (!data.endpointId && self.endpointId) {
        ...
        var wsBody = {
            ...
            headers: _.defaults(data ? data.headers || { } : { }, whichHeader)
        };

        if (data) {
            wsBody.data = _.clone(data);
            delete wsBody.data.headers;
        }

        debug('socket send ' + httpMethod, self.baseURL + urlPath, data);
    ...
    self.close = function (callback) {
        self.wsCall('delete', '/endpointconnections/' + self.connectionId, {
    ...
    self.messages.send = function (params, callback) {
        if (!params.to && !params.connectionId) {
            callback(new Error("Cannot send message without to (endpointId) or connectionId"));
            return;
        }
        params.endpointId = params.endpointId || self.endpointId;
        self.wsCall('post', '/messages', params, callback);
    };
    ...
    self.groups.publish = function (params, callback) {
        ...
        self.wsCall('post', '/channels/' + params.groupId + '/publish', {
            endpointId: self.endpointId,
    ...
    self.groups.join = function (params, callback) {
        ...
        self.wsCall('post', '/channels/' + params.groupId + '/subscribers', body, callback);
    };
    /**
     * Leave a group.
     * @param {object} params
    ...
    self.groups.leave = function (params, callback) {
        ...
        self.wsCall('delete', '/channels/' + params.groupId + '/subscribers', body, callback);
    };
    ...
    self.roles.get = function (opts, callback) {
        ...
        opts = opts || {};
        if (!opts.appId) {
            return callback(new Error('appId is required to retrieve roles'));
        }
        var requestOptions = _.defaults(opts || {}, {
