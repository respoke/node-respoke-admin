...
function Client(options) {
    ...
    self.auth.connect = function (opts) {
        ...
        self.socket.on('connect', function () {
            ...
            self.wsCall('post', '/connections', dataBody, function (err, data) {
                if (err) {
                    self.emit('error', err);
                    return;
                }
    ...
    self.close = function (callback) {
        ...
        self.wsCall('delete', '/connections/' + self.connectionId, {
            endpointId: self.endpointId
        },
        // note: this callback no longer seems to be getting called.
        function (err) {
            if (err) {
    ...
    self.presence.observe = function (endpoints, callback) {
        self.wsCall('post', '/presence-observers', {endpointList: endpoints}, callback);
    };
    /**
    ...
    self.presence.set = function (params, callback) {
        ...
        self.wsCall('post', '/presence', body, callback);
    };
    ...
    self.roles.delete = function (opts, callback) {
        opts = opts || {};
        if (!opts.roleId) {
            return callback(new Error("Cannot delete role when roleId is " + opts.roleId));
        }
