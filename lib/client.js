...
function Client(options) {
    ...
    self.auth.connect = function (opts) {
        ...
        self.socket.on('connect_error', function (err) {
            debug('socket connect_error', err);
            /**
     ...
     * 
     * ##### Why is it `tokenId` instead of `token`?
     * Technically you are creating a `token` which can be retrieved by the user. They 
     * use the ID of the token, `tokenId`, to retrieve it.
     * 
     * @param {object} opts
    ...
    self.auth.developmentMode = function (opts, callback) {
        ...
        if (!opts.appId) {
            return callback(new Error("Cannot authenticate without appId"));
        }
