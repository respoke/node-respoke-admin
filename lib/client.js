...
function Client(options) {
    ...
    self.wsCall = function (httpMethod, urlPath, data) {
        ...
        self.socket.emit(httpMethod.toLowerCase(), JSON.stringify(wsBody), function (response) {
            try {
                ...
                deferred.reject(new errors.UnparseableResponse());
                return;
            }
            if (response && response.error) {
                debug('socket response error', response);
                deferred.reject(new errors.SocketErrorResponseFromServer(
                    response,
                    httpMethod,
                    urlPath
                ));
    ...
    self.close = function (callback) {
        ...
        self.wsCall(
            ...
            if (err) {
                ...
                deferred.reject(new errors.FailedEndpointDeletion());
            }
        });
    ...
    self.presence.observe = function (endpoints, callback) {
        ...
        self.wsCall('post', '/presence-observers', { endpointList: endpoints })
            .then(function (response) {
                deferred.resolve(response);
            }).catch(function (error) {
    ...
    self.presence.set = function (params, callback) {
        ...
        self.wsCall('post', '/presence', body)
            .then(function (response) {
                deferred.resolve(response);
            }).catch(function (error) {
                deferred.reject(error);
    ...
    self.messages.send = function (params, callback) {
        ...
        self.wsCall('post', '/messages', params)
            .then(function (response) {
                deferred.resolve(response);
            }).catch(function (error) {
                deferred.reject(error);
