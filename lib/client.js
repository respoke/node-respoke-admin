...
Client.prototype.connect = function () {
    ...
    var base = url.parse(self.baseURL);
    ...
    var connectParams = {
        ...
        query: 'app-token=' + self.options.token
    };
    ...
    request({
        ...
        var connectionString = self.baseURL.replace('/v1','') + '?app-token=' + self.appToken;
        // console.log(connectionString, connectParams);
        // self.socket = io.connect(connectionString, connectParams);
        self.socket = io.connect(connectionString);
        
        self.socket.on('connect', function(data) {
            console.log('inside connect', data);
        ...
        self.socket.on('connect_error', function (res, arg2) {
            self.emit('error', res);
        });
